...
async def _(msg: Message, state: FSMContext):
    if msg.from_user.id == settings.OWNER_ID:
        ...
        await msg.answer('<b>Напишите сообщение которое хотите отправить всем</b>', parse_mode='html')


@router.message(SendToAllAdmin.text)
async def _(msg: Message, state: FSMContext):
    users = await get_users()
    await msg.send_copy(settings.OWNER_ID)
    await asyncio.sleep(15)
    for user in users:
        if user.tg_id not in settings.BLACKLISTED_USERS:
            try:
...
async def _(msg: Message):
    await msg.answer("Админ-панель", reply_markup=kb.admin_keyboard)